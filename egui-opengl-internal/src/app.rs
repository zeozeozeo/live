...
use windows::Win32::{
    ...
    UI::{
        ...
        WindowsAndMessaging::{GetClientRect, WM_SIZING},
    },
};
...
impl<T> OpenGLApp<T> {
    ...
    pub fn render(&self, hdc: HDC) {
        unsafe {
            ...
            let dpi = match GetDpiForWindow(WindowFromDC(hdc)) {
                0 => 96.0,
                dpi => dpi as f32,
            };
            ...
            let clipped_shapes = this.ctx.tessellate(output.shapes, pixels_per_point);
